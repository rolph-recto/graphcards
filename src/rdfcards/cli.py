"""Argparse commands and terminal interaction for RDFCards."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="rdfcards", description="Learn RDF triples and entities with SPARQL-driven cards"
    )
    parser.add_argument(
        "-c", "--config", default="rdfcards.toml", help="project TOML file (default: %(default)s)"
    )
    ...
    *,
    ...
    try:
        if args.command == "init":
            ...
            if args.template:
                print(
                    f"Created RDFCards workspace from template {args.template!r} in {directory}",
                    file=output,
                )
            else:
                print(f"Created empty RDFCards workspace in {directory}", file=output)
            return 0
        ...
        return 130
    except (RdfCardsError, OSError, sqlite3.Error) as command_error:
